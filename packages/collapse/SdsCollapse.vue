        ...
        <template slot="title">
          <div @click="pauseEvent">
            <span class="sds-collapse__icon-prefix">
              <slot name="prefix"><i class="sds-collapse__icon" v-if="!!item.prefixIcon" :class="item.prefixIcon"></i></slot>
            </span>
            <span class="sds-collapse__title" v-if="item.title" @click="activeClick(item.title)">{{item.title}}</span>
            <div v-else-if="!!item.titleList && !item.title && item.titleList.length !== 0">
              <div class="sds-collapse__custom-title">
                <div class="sds-collapse__title-item" v-for="titleItem in item.titleList" v-bind:key="titleItem.id" @click="!titleItem.click ? '' : activeClick(titleItem)">
            ...
            <span class="sds-collapse__icon-suffix">
              <slot name="suffix"><i class="fa fa-caret-down" v-if="!!item.suffixIcon" :class="item.suffixIcon"></i></slot>
            </span>
          </div>
        </template>
